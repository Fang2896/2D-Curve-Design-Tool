...
Eigen::VectorXd UtilFunc::rbfInterpolateCoeff(const float &sigma, const Eigen::VectorXd &xvals, const Eigen::VectorXd &yvals) {
    int n = (int)xvals.size();
...
// kernel function
float UtilFunc::computeKernelGaussianValue(const float &x, const float &mean, const float &sigma) {
    float diff = x - mean;
...
Eigen::VectorXd UtilFunc::polynomialRegressionCoeff(const Eigen::VectorXd &xvals, const Eigen::VectorXd &yvals, int order, float lambda) {
    int n = (int)xvals.size();

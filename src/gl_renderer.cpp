...
GLRenderer::GLRenderer(CurveModel& model, QWidget* parent, int width,
                       int height)
...
void GLRenderer::paintGL() {
    ...
    if (m_model.getPolyInterCurveStatus()) {
        ...
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getPolyInterCurveDataSize());
    ...
    if (m_model.getRBFInterCurveStatus()) {
        ...
        core->glLineWidth(4.0f);
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getRBFInterCurveDataSize());
        core->glBindVertexArray(0);
    }
    ...
    if (m_model.getPolyRegreCurveStatus()) {
        ...
        core->glLineWidth(4.0f);
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getPolyRegreCurveDataSize());
        core->glBindVertexArray(0);
    }
    ...
    if (m_model.getUniformParamCurveStatus()) {
        ...
        core->glLineWidth(4.0f);
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getUniformParamCurveDataSize());
        core->glBindVertexArray(0);
    }
    ...
    if (m_model.getChordalParamCurveStatus()) {
        ...
        core->glLineWidth(4.0f);
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getChordalParamCurveDataSize());
        core->glBindVertexArray(0);
    }
    ...
    if (m_model.getCentrietalParamCurveStatus()) {
        ...
        core->glLineWidth(4.0f);
        core->glDrawArrays(GL_LINE_STRIP, 0,
                           m_model.getCentrietalParamCurveDataSize());
        core->glBindVertexArray(0);
    }
    ...
    if (m_model.isBezierMode) {
        if (m_model.getBezierCurveStatus()) {
            ...
            core->glDrawArrays(GL_LINE_STRIP, 0,
                               m_model.getBezierCurveDataSize());
            core->glBindVertexArray(0);
        }
        ...
        if (m_model.getBezierControlLineStatus()) {
            ...
            core->glDrawArrays(GL_LINES, 0,
                               m_model.getBezierControlLineDataSize());
            core->glBindVertexArray(0);
        }
    }
...
void GLRenderer::updatePointsBuffer() {
    ...
    core->glBufferData(GL_ARRAY_BUFFER,
                       sizeof(QVector2D) * m_model.getPointsSize(),
    ...
    core->glBufferData(GL_ARRAY_BUFFER,
                       sizeof(QVector3D) * m_model.getPointsSize(),
...
void GLRenderer::updateCurveBuffer() {
    ...
    core->glBufferData(GL_ARRAY_BUFFER,
                       sizeof(QVector2D) * m_model.getPolyInterCurveDataSize(),
    ...
    core->glBufferData(GL_ARRAY_BUFFER,
                       sizeof(QVector2D) * m_model.getRBFInterCurveDataSize(),
    ...
    core->glBufferData(GL_ARRAY_BUFFER,
                       sizeof(QVector2D) * m_model.getPolyRegreCurveDataSize(),
    ...
    core->glBufferData(
        GL_ARRAY_BUFFER,
        sizeof(QVector2D) * m_model.getUniformParamCurveDataSize(),
    ...
    core->glBufferData(
        GL_ARRAY_BUFFER,
        sizeof(QVector2D) * m_model.getChordalParamCurveDataSize(),
    ...
    core->glBufferData(
        GL_ARRAY_BUFFER,
        sizeof(QVector2D) * m_model.getCentrietalParamCurveDataSize(),
    ...
    if (m_model.isBezierMode) {
        ...
        core->glBufferData(GL_ARRAY_BUFFER,
                           sizeof(QVector2D) * m_model.getBezierCurveDataSize(),
        ...
        core->glBufferData(
            GL_ARRAY_BUFFER,
            sizeof(QVector2D) * m_model.getBezierControlLineDataSize(),
...
void GLRenderer::initBuffers() {
    ...
    core->glBindBuffer(GL_ARRAY_BUFFER, pointsColorVBO);
    core->glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float),
                                (void*)nullptr);
...
void GLRenderer::mousePressEvent(QMouseEvent* event) {
    ...
    if (event->modifiers() & Qt::ControlModifier &&
        ...
        selectedPointIndex =
            m_model.findNearestPointInRange({spaceX, spaceY}, PICK_RADIUS);

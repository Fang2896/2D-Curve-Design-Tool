...
QVector2D Point::getPosition() const {
    return m_position;
}
void Point::setPosition(const QVector2D& position) {
    m_position = position;
}

// getter and setter for color
QVector3D Point::getColor() const {
    return m_color;
}
void Point::setColor(const QVector3D& color) {
    m_color = color;
}
...
void Points::addPoint(QVector2D pos) {
    ...
    qDebug() << "Add New Point, position: [" << pos
             << "], id : " << newPoint.getID();
